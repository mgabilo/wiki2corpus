...
#
# Modified by Mark Sammons 6/30/08: added locale info (not sure this does
#   much by itself) and OK_CHARS filter (removes all but ascii text)
#   to avoid problems with input containing non-standard character codes
#
# Modified by Michael Gabilondo June 2009: reverted last change.
#
...
#setlocale(LC_CTYPE, "en_us_iso88591");
...
while (!eof INPUT)
{
   ...
   #$newline =~ s/[^$OK_CHARS]//gxms;
   if (substr($newline, 0, 8) eq "\@Article") {
